...
void estadisticas (){
    ...
    cout<<"Nuevo Hito S.A.C. \t"<<nuevoHito_SAC[empieza]<<" PDV\t\t"<<nuevoHito_SAC[noEmpieza]<<" PDV"<<endl;
    cout<<"-----------------------------------------------------------"<<endl;
    ///SUMA EL TOTAL DE PUNTOS DE CADA JUGADOR
    for(int i=0; i<2; i++){
        total[i] = puntos_estatuillas[i] + puntos_estatuillas_primerIntento[i] + puntos_ganador_faseFinal[i] + puntos_ganador_sinEstatuillas[i] + puntos_rivalObtiene[i] + puntos_lanzamientoJ[i] + nuevoHito_SAC[i];
    }
